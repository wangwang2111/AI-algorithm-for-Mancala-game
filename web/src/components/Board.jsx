...
import {
  animateSow as animateFanout,
  animateCapture as doCapture,
} from "../ui/sowing";
...
function updateCountsText(root, s) {
  ...
  for (let side = 0; side <= 1; side++) {
    ...
    for (let i = 0; i < 6; i++) {
      const face =
        root.querySelector(
          `.pit__face[data-side="${side}"][data-index="${i}"]`
        ) ||
        (side === 0
  ...
  const st0Face =
    root.querySelector(`.board__store[data-owner="0"] .store__face`) ||
    root.querySelector(".board__store--right .store__face") ||
    root.querySelector(".board__store--right .store");
  const st1Face =
    root.querySelector(`.board__store[data-owner="1"] .store__face`) ||
...
function applyTurnVisuals(root, player) {
  if (!root) return;
  root.classList.toggle("turn-0", player === 0);
  root.classList.toggle("turn-1", player === 1);
}
...
function hydrateFromState(boardEl, state) {
  ...
  // small helpers
  const countSeeds = (sc) =>
    sc ? sc.querySelectorAll(".pit__seed, .store__seed").length : 0;

  // remove some stones (take from the end so visuals look natural)
  const removeN = (sc, n) => {
    if (!sc || n <= 0) return;
    const list = [...sc.querySelectorAll(".pit__seed, .store__seed")];
  ...
  bottomFaces.forEach((face, i) => {
    ...
    if (!scatter) return;
    const want = state.pits?.[0]?.[i] ?? 0;
    const have = countSeeds(scatter);
    const diff = want - have;
    if (diff > 0) {
      const ctl = getScatterController(scatter);
      ctl.add(diff);
    } else if (diff < 0) {
      removeN(scatter, -diff);
    }
  });
  ...
  // Stores
  const rightStoreFace = boardEl.querySelector(
    ".board__store--right .store__face"
  );
  const leftStoreFace = boardEl.querySelector(
    ".board__store--left .store__face"
  );
  ...
  if (rightStoreFace) {
    ...
    if (sc) {
      const cx = sc.clientWidth / 2,
        cy = sc.clientHeight / 2,
        r = 36;
      const ctl = getScatterController(sc, {
        centerExclusion: { x: cx, y: cy, r },
      });
      const want = state.stores?.[0] ?? 0;
      const have = countSeeds(sc);
      const diff = want - have;
      if (diff > 0) ctl.add(diff);
      else if (diff < 0) removeN(sc, -diff);
    }
  }
  if (leftStoreFace) {
  ...
  // reflect whose turn (works for tutorial DOM-only hydration)
  applyTurnVisuals(boardEl, state.current_player ?? 0);
}

const Board = forwardRef(function Board({ state, canPlay, onPlay }, apiRef) {
  if (!state) return null;
  const pits =
    Array.isArray(state?.pits) && state.pits.length === 2
      ? state.pits
      : [Array(6).fill(0), Array(6).fill(0)];

  const stores =
    Array.isArray(state?.stores) && state.stores.length === 2
      ? state.stores
      : [0, 0];

  const [p0, p1] = pits;
  const [st0, st1] = stores;
  const turn = Number.isInteger(state?.current_player)
  ...
  React.useEffect(() => {
    ...
    if (root) applyTurnVisuals(root, turn);
  }, [turn]);
  ...
  React.useEffect(() => {
    ...
    function onFirstInteraction() {
      unlockAudio();
      root.removeEventListener("pointerdown", onFirstInteraction);
      root.removeEventListener("click", onFirstInteraction);
    }
    root.addEventListener("pointerdown", onFirstInteraction, { once: true });
    root.addEventListener("click", onFirstInteraction, { once: true });
  ...
  React.useEffect(() => {
    ...
    const onSeedLanded = (e) => {
      const isStore =
        e?.detail?.isStore ||
        !!(
          e?.detail?.toEl?.closest?.(".store__face") ||
          e?.detail?.toEl?.closest?.(".store")
        );
      if (isStore) sfx.seedStore();
      else sfx.seedPit();
    };

    root.addEventListener("mancala:seed-landed", onSeedLanded);
    return () => root.removeEventListener("mancala:seed-landed", onSeedLanded);
  }, []);
  ...
  function getFullRing() {
    ...
    const bottom = [
      ...boardRef.current.querySelectorAll(".board__pits--bottom .pit__face"),
    ];
    const rightStore =
      boardRef.current.querySelector(".board__store--right .store__face") ||
      boardRef.current.querySelector(".board__store--right .store");
    const top = [
      ...boardRef.current.querySelectorAll(".board__pits--top .pit__face"),
    ].reverse();
    const leftStore =
      boardRef.current.querySelector(".board__store--left .store__face") ||
      boardRef.current.querySelector(".board__store--left .store");
  ...
  function buildPathFrom(player, startPitIdx) {
    ...
    const startIndexInRing = player === 0 ? startPitIdx : 7 + startPitIdx;

    const rotated = ring
      .slice(startIndexInRing)
      .concat(ring.slice(0, startIndexInRing));
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      hydrate: (s) => {
        ...
        const tick = () => {
          if (ready() || Date.now() > deadline) {
            ...
            updateCountsText(root, s);
            applyTurnVisuals(root, s.current_player ?? 0);
            return;
          }
      ...
      setDemoState: (demo) => {
        ...
        applyTurnVisuals(root, demo.current_player ?? 0);
      },
      setTurn: (player) => {
        const root = boardRef.current;
        if (!root) return;
        applyTurnVisuals(root, player);
      },
      setBusy,
    }),
    [state]
  ...
  async function handleSowAndApply(
    player,
    startPitIdx,
    stagger = 300,
    hopMs = 650
    ...
    setBusy(true);
    try {
      await apiRef.current?.animateSowFromPit?.(
        player,
        startPitIdx,
        seedCount,
        { stagger: stagger, hopMs: hopMs }
      );
